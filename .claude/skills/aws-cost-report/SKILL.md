...
2. **Cost Explorer was not yet enabled on the account.** Visiting the Billing console auto-enabled it, but AWS documents up to a 24-hour delay before historical data backfills. The very first live run against this account hit that real, undocumented-until-now state — `GetCostAndUsage` returned `DataUnavailableException` for every section. The script handled it exactly as designed: each section's real error landed in `errors`, the report still returned valid JSON with honest zeros and empty lists, and nothing was fabricated to look like a complete report.
3. **A follow-up run roughly an hour later showed the backfill had cleared.** Both `daily_cost_by_service` and `month_over_month` now return successfully with no errors — but `total_unblended_cost` is genuinely `0`, because this is a fresh AWS free-tier account ("credits cover your free plan costs," per the Billing console) with no billable resources actually deployed yet. This is a third distinct, honest state worth naming explicitly: not "Cost Explorer unavailable" and not "a real anomaly fired," but "the query succeeded and correctly found nothing" — exactly what a clean, unused account should report, and different from either of the other two states. `test-fixtures/sample-output.json` reflects this later, cleaner run.
...
| Query succeeds with genuinely zero cost (clean, unused account) | ✅ (real run ~1h later, both sections error-free, `total_unblended_cost: 0`) | — |
| Daily cost by service, a real **non-zero** result | ❌ still pending — this account has no billable usage at all yet | — |
| Month-over-month comparison, a real **non-zero** result | ❌ still pending — same reason | — |
| `detect_service_anomalies`: a real percentage-and-dollar-floor anomaly | ❌ no real cost data exists yet to test against | ✅ `test-fixtures/test_aws_cost_report.py` (real anomaly, near-zero swing correctly suppressed by the dollar floor, same data correctly flagged once the floor is lowered below it, brand-new-service no-prior-average case, cross-service independence, clean/stable case, exact-50%-boundary non-fire) |
...
**What's still open:** the happy path (real, non-zero cost data flowing through `daily_cost_by_service` and `month_over_month`, and a genuine anomaly firing) has not yet been observed live — this specific account has no deployed billable resources at all, so there's nothing for Cost Explorer to report yet. That's a property of the test account, not a flaw in the skill: the graceful-empty-result path is now proven correct, same as the graceful-error path. Confirming the non-zero happy path requires either real usage accruing on this account or testing against a different, active AWS account.
