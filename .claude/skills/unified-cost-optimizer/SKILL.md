...
**Coverage gap in the query-performance section, found during demo testing (2026-07-15):** this skill calls `query_optimizer.py` with no `--query-id`, which only analyzes the **top 10 slowest queries by duration** in the window. A query with a real flagged issue (e.g. the account's known `spilling_to_local_storage` case, 26MB spilled, 1.46s execution) can rank well outside the top 10 by duration if other unrelated queries simply ran longer without any issue at all -- confirmed live: in a 14-day window, the top 10 slowest queries ranged 1.9s-13.8s and none included the known-flagged 1.46s query, so this section reported zero query-performance findings despite a real one existing in the account. This is not a detection bug -- `diagnose()` correctly flags a query whenever it's actually examined -- it's a blind spot in *which* queries get examined by default. Duration-ranking and issue-worthiness are not the same ordering. Not yet fixed; a future version could rank by "has any flagged issue" first, duration as a tiebreaker, or widen the default scan.
