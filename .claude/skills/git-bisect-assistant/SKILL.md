...
1. Identify a **known-good ref** (a commit/tag where the behavior was correct) and a **known-bad ref** (default `HEAD`, where it's currently broken), and a **test command** that exits `0` when the behavior is correct and non-zero when it's broken. If the user hasn't specified these, ask — this skill cannot guess what "good" and "bad" mean for their code.
   - **The test command runs from the worktree root (= repo root), not from wherever the skill was invoked.** Use repo-root-relative paths in the command (e.g. `python subdir/test_x.py`, not `python test_x.py` from inside `subdir/`). A wrong path fails identically at *every* commit (file-not-found, non-zero exit), which bisect reads as "always bad" and converges on a **wrong culprit with full confidence** — found live during verification, where a cwd-relative path produced a false convergence two commits away from the real bug. If every step in the step log shows the same unusual exit code (e.g. `2` from Python, meaning the script never ran), suspect the command path before trusting the result.
...
4. **Combined complex history** — 9 mainline commits with the bug buried at commit 5, plus a 2-commit side branch merged in via `--no-ff` *between* the bug and `HEAD`. Converged in 4 steps to the exact culprit; the merge commit and both side-branch commits were never even tested (binary search correctly skipped them as irrelevant to the failing file). Also re-confirmed the isolation claim under the hardest condition: the invoking checkout *was* the branch containing the bug, and it was untouched throughout. This same run is what surfaced the working-directory trap now documented in Steps: the first attempt used a cwd-relative test path and falsely converged on an innocent commit before the path was corrected.
5. **A genuinely flaky test command — closed 2026-07-20, and it confirmed the danger, not just the cap.** Built a real 6-commit history with a real, deterministic bug at commit 4, and a test script with a genuine 35% chance of reporting the *opposite* of the real result regardless of the actual code state (confirmed by running the test 10 times at both the good and bad commit and observing a real mixed pass/fail distribution at each — not a simulated flip, an actual `random.random()` in the test file). Ran the real bisect assistant against this repo **three separate times**: it converged (`converged: true`, no error, no warning) on **three different wrong commits** — never once landing on the real bug at commit 4. This is the exact danger the caveat below already named theoretically, now proven to actually happen: a flaky test doesn't just risk one wrong answer, it can hand back a *different* wrong answer with full false confidence on every run. There is no cap or retry count that catches this — the loop behaves identically whether the verdicts it receives are true or false.
...
| Genuinely flaky (non-deterministic) test command | ✅ **confirmed dangerous, not just theoretical** — 3 real runs against a real 35%-flip-rate test, 3 different wrong culprits, zero correct results, zero indication anything was wrong |
| Test command with external side effects | ❌ — the worktree isolates git state only, not databases/networks the test touches |

## The confirmed danger of a flaky test command (read before trusting a "converged" result)

A flaky/non-deterministic test command is not a theoretical risk this skill merely warns about — it was reproduced live. **The single most important thing to know: `converged: true` means bisect received a consistent enough sequence of verdicts to narrow down to one commit — it does NOT mean those verdicts were correct.** If a test command is even moderately non-deterministic, this skill (like `git bisect` itself, which it faithfully orchestrates) has no way to detect that and will report a wrong culprit with exactly the same confidence as a right one. Before trusting a result: run the test command manually several times against both the reported culprit and its parent commit, and confirm the verdict is actually stable. If it isn't, the result is not reliable regardless of what the JSON says.
