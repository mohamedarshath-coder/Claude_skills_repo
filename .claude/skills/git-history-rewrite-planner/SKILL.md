...
4. Inside the disposable clone, it scans **two separate surfaces** for each pattern: every commit's full file tree (not just diffs — a diff-only search would miss a commit that inherits the secret unchanged from a parent), and every commit/tag's full message text — these require different git mechanisms to both find and fix, and checking only one leaves the other silently unverified (see the live bug below).
5. If nothing is found: reports `no_matches_found_nothing_to_purge` immediately. Nothing else runs.
6. If found, this is the **Task Loop**: each round applies `git filter-repo --replace-text` (file content) **and** `--replace-message` (commit/tag messages) with the supplied patterns, then **independently re-scans both surfaces of the rewritten disposable clone from scratch** to verify zero matches remain — never trusting the rewrite tool's exit code alone, the same "verify against real ground truth every round" discipline as `snowflake-ddl-diff`. Converges the moment verification finds zero matches on both surfaces; hard-caps at `--max-iterations` (default 3) per `.claude/rules/loop-engineering.md`.
...
**Three real bugs found and fixed before this was ever trustworthy:**
...
3. **Commit messages are a real, separate leak surface `--replace-text` never covers.** See below — found on a more complex real test and fixed the same way as the two above: by directly inspecting the actual result rather than trusting a clean-looking summary.
...
**A third real bug was found on a genuinely more complex scenario** (2 secrets, a real branch + merge, a decoy value, and one secret deliberately also pasted into a commit message): `git filter-repo --replace-text` **does not rewrite commit messages at all** — that requires the separate `--replace-message` flag. The scanner (`git grep`) never checks commit messages either, only file trees. Together, this meant the first version of this skill would run the redaction, re-scan file content, find nothing, and confidently report `verified_clean_after_redaction` — while the secret was still sitting untouched in a rewritten commit's message. Found by manually reproducing the exact clone-and-redact steps and directly inspecting the rewritten commit message, since the disposable clone is deleted before a human ever gets to look at it. Fixed with a dedicated `scan_commit_messages()` check (added to every scan, not just the initial one) and by passing `--replace-message` alongside `--replace-text` in every redaction call. Re-verified: the same secret in the same commit message is now correctly found, correctly redacted, and correctly confirmed absent — independently checked by hand, not just trusting the tool's own "converged" claim a second time.

The same complex scenario also confirmed real precision: the decoy value (`CorrectHorseBattery00`, similar to but distinct from the real secret `CorrectHorseBattery99`) was never touched, and the secret introduced on a merged-in feature branch was correctly found and purged across the real branch/merge structure (6 real commits, not just a linear chain).
